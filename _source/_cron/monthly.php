<?php
	/* 
		...
		File Description:
			Monthly Cronjob File - Set this up on your server to run at the specific interval!
	*/
	...
	ob_start();
	echo "// Monthly bugfishCMS Cronjob".$php_space.$php_space;
			   ...
			   foreach (glob($object["path"]."/_site/".$file."/_cron/_monthly/cron.*.php") as $filename){ 
					...
					foreach ($object["extensions_path"] as $filename) {
						...
						if (is_dir($filename."/_cron/_monthly")) {
								foreach (glob($filename."/_cron/_monthly/cron.*.php") as $filenamex){ require_once $filenamex; }
						}
					}
