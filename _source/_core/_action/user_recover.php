<?php
	...
	*/ if(file_exists("../../settings.php")) { require_once("../../settings.php"); } else { @http_response_code(404); Header("Location: ../"); exit(); } 
	...
	if(defined("_HIVE_USR_REC_DISABLE_")) { 
		if(_HIVE_USR_REC_DISABLE_) { 
	...
	if(defined("_HIVE_USR_REC_REFER_")) { 
		if(_HIVE_USR_REC_REFER_) {
			$refer = trim(_HIVE_USR_REC_REFER_ ?? '');
			if(strpos(_HIVE_USR_REC_REFER_, "?") > 0) { $refer = _HIVE_USR_REC_REFER_."&"; }
				else { $refer = _HIVE_USR_REC_REFER_."?"; }
			Header("Location: ".$refer."rec_token=".@$_GET["rec_token"]."&rec_user=".@$_GET["rec_user"]);
			exit();
		} else {
			hive__simple_start($object, "Module Error", ''); ?>
	...
	<p>_HIVE_USR_REC_REFER_ needs to be set to use this action script!
	</p>
			</div>	
			<?php hive__simple_end($object, false);
		}
	} else { 
		hive__simple_start($object, "Module Error", ''); ?>
