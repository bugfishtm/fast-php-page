<?php
	...
	*/
	if(file_exists("./settings.php")) {
		...
		if(!defined("_HIVE_BUILD_") OR !defined("_HIVE_VERSION_") OR !defined("_HIVE_RNAME_")) {
			hive_error_full("Critical Error", "Something went wrong initializing the Side Module: '".@htmlspecialchars(_HIVE_MODE_ ?? '')."'!", "Please check the versioning informations of the deployed module.", true, 503);
		}
		if(_HIVE_BUILD_ == 0 OR _HIVE_VERSION_ == 0 OR _HIVE_RNAME_ == 0) {
			hive_error_full("Critical Error", "Something went wrong initializing the Side Module: '".@htmlspecialchars(_HIVE_MODE_ ?? '')."'!", "Please check the versioning informations of the deployed module.", true, 503);		
		}
		
		// Check Update and Build Number/RNAME
		if(_HIVE_BUILD_ != _HIVE_BUILD_ACTIVE_ OR _HIVE_VERSION_ != _HIVE_VERSION_ACTIVE_ OR _HIVE_RNAME_ != _HIVE_RNAME_ACTIVE_) {
			// Update is needed
			hive_error_full("Update Required", "You need to visit <a href='./updater.php'>./updater.php</a> for: '".@htmlspecialchars(_HIVE_MODE_ ?? '')."'!", "Version/Build or RNAME in Database does not fit the current running site module.<br /> You may need to update this website to proceed!", true, 503);
		} 	
