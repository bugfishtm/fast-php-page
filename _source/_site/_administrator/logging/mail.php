<?php
	...
	*/
	...
	if(!hive__access($object, "admin_logging", false) AND $object["user"]->user["user_initial"] != 1) {
		...
		////////////////////////////////////////////////////////////////
		$title_general 	= "Mail";
		$trts_general 	= "creation, receiver, debugmsg, id";
		$item_general 	= "Mail Log Entry";
		$text_general 	= "Review and manage mail log entries for your website's communication activities in this dedicated section.";
		$table_general 	= _TABLE_LOG_MAIL_;
		...
		if(is_array($value_array)) {
			foreach($value_array as $key => $value) {
				...
				$value_array[$key]["receiver"] = @htmlspecialchars($value_array[$key]["receiver"]);
				$value_array[$key]["debugmsg"] = @htmlspecialchars($value_array[$key]["debugmsg"]);
			}
		}
