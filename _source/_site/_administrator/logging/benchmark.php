<?php
	...
	*/
	...
	if(!hive__access($object, "admin_logging", false) AND $object["user"]->user["user_initial"] != 1) {
		...
		// Variables
		////////////////////////////////////////////////////////////////
		$title_general 	= "Benchmark";
		$trts_general 	= "full_url, value_time, value_memory, value_load, value_queries, section, id";
		$item_general 	= "Benchmark Entry";
		$text_general 	= "Access the benchmark page to gauge and analyze performance metrics, empowering informed decision-making for your website's optimization.";
		$table_general 	= _TABLE_LOG_BENCHMARK_;
		...
		if(is_array($value_array)) {
			foreach($value_array as $key => $value) {
				...
				$value_array[$key]["value_time"] = @htmlspecialchars($value_array[$key]["value_time"])."s";
				$value_array[$key]["value_memory"] = @htmlspecialchars($value_array[$key]["value_memory"]);
				$value_array[$key]["value_load"] = @htmlspecialchars($value_array[$key]["value_load"]);
				$value_array[$key]["value_queries"] = @htmlspecialchars($value_array[$key]["value_queries"]);
